...
![Author](https://img.shields.io/badge/author-@wahyudi-black)
...
> [!NOTE]
> Fokus repo ini adalah dokumentasi implementasi dan operasional. Untuk rujukan konsep, kompatibilitas, dan parameter resmi, lihat bagian [Dokumentasi resmi](#dokumentasi-resmi) di akhir README.
...
```mermaid
flowchart LR
    Admin["Admin / Client"] -->|HTTPS Public API| EXT["external-openstack<br/>172.16.4.0/24"]
    EXT --> HZN["Horizon / Public API"]

    MAAS["MAAS<br/>PXE / DHCP / DNS / Provisioning"] --> MGMT["management<br/>172.16.1.0/24"]
    MGMT --> JUJU["Juju Controller & Agents"]
    JUJU --> MODEL["OpenStack Model"]

    subgraph CONTROL["HA Control Plane"]
        RMQ["RabbitMQ"]
        DB["Database / Galera"]
        KEY["Keystone"]
        NEU["Neutron API"]
    ...
    MODEL --> CONTROL
    CONTROL <-->|Service-to-service API| INT["internal-openstack<br/>172.16.2.0/24"]
    CONTROL <-->|Metrics / Alarm| TEL["Telemetry<br/>Gnocchi / Aodh / Ceilometer"]

    subgraph COMPUTE["Compute + Storage"]
        COMP1["Comp-01<br/>Nova Compute + Ceph OSD"]
        COMP2["Comp-02<br/>Nova Compute + Ceph OSD"]
    end

    CONTROL <-->|Storage traffic| STG["storage-network<br/>172.16.5.0/24"]
    STG --> CEPH["Ceph MON / OSD"]
    COMPUTE --> CEPH

    VM["Tenant VM"] -->|Floating IP / Provider access| PHY["provider-network<br/>172.16.3.0/24"]
    COMPUTE --> VM
```
...
| 4 | [Deployment Charm](HA%20OpenStack%20Charm/Deployment%20Charm.md) | Deployment service OpenStack menggunakan Juju charms. |
